...
class Bunyad_PbBasic_Separator extends WP_Widget
{
	public function __construct()
	{
		parent::__construct(
			...
			__('Separator - Space or Line', 'bunyad'),
			array('description' => __('Create an empty space or line separator.', 'bunyad'))
		);
	}
	...
	<p><strong><?php _e('Important:', 'bunyad'); ?></strong> <?php 
		_e('This block automatically shows the sub-categories under a parent category. Only a parent category needs to be selected.', 'bunyad'); ?></p>
	
	<p>
		<label><?php _e('Separator Type:', 'bunyad'); ?></label>
		<select class="widefat" name="<?php echo esc_attr($this->get_field_name('type')); ?>">
			<option value="line"><?php _e('Line', 'bunyad'); ?></option>
			<option value="space"><?php _e('Space', 'bunyad'); ?></option>
			<option value="half-line"><?php _e('Half Height Line', 'bunyad'); ?></option>
			<option value="half-space"><?php _e('Half Height Space', 'bunyad'); ?></option>
		</select>
	</p>
...
class Bunyad_PbBasic_RichText extends WP_Widget {
	...
	public function __construct()
	{
		parent::__construct(
			...
			__('Rich Text - Visual Editor', 'bunyad'),
			array('description' => __('Visual editor for text and HTML.', 'bunyad'))
		);
	}
...
?>
		<input type="hidden" class="<?php echo $this->get_field_name('the_id'); ?>" value="<?php echo $this->get_field_id('text'); ?>" />

		<div>
	
		<?php 
			$settings = array('media_buttons' => true, 'textarea_name' => $this->get_field_name('text'));
			wp_editor($text, $this->get_field_id('text'), $settings);
		?>
        </div>
		...
		<p>
			...
			checked(isset($instance['filter']) ? $instance['filter'] : 0); ?> />&nbsp;<label for="<?php echo $this->get_field_id('filter'); ?>"><?php
				 _e('Automatically add paragraphs'); ?></label>
		 </p>
		...
		jQuery(function($) {
			...
			var Bunyad_PbBasic_RichText = {
				...
				set_ids: function() {
					...
					self.the_ID = '<?php echo esc_attr($this->get_field_id('text')); ?>';
				},
				...
				open: function()
				{
					...
					var init = tinyMCEPreInit.mceInit[self.the_ID],
						qt_init = tinyMCEPreInit.qtInit[self.the_ID];
